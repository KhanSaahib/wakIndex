...
def _parser() -> argparse.ArgumentParser:
    ...
    check.add_argument("--policy", default="agentscope-policy.toml")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "ui":
        from agentscope.ui import serve
    ...
    if not policy_path.is_file():
        print(f"Policy not found: {policy_path}; run 'agentscope init'", file=sys.stderr)
        return 1
